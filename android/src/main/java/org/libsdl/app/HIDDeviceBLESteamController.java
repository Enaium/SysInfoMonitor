...
class HIDDeviceBLESteamController extends BluetoothGattCallback implements HIDDevice {
    ...
    static class GattOperation {
        ...
        private GattOperation(BluetoothGatt gatt, Operation operation, UUID uuid, int delayMs) {
            mGatt = gatt;
            mOp = operation;
            mUuid = uuid;
            mDelayMs = delayMs;
        ...
        private GattOperation(BluetoothGatt gatt, Operation operation, UUID uuid, byte[] value, int delayMs) {
            mGatt = gatt;
            mOp = operation;
            mUuid = uuid;
            mValue = value;
    ...
    private void enableNotification(UUID chrUuid) {
        ...
        GattOperation op = GattOperation.enableNotification(mGatt, chrUuid, 500);
        ...
        mHandler.postDelayed(new Runnable() {
            ...
            public void run() {
                if (!finalThis.mHasEnabledNotifications) {
                    ...
                    // Give one more try.
                    GattOperation retry = GattOperation.enableNotification(finalThis.mGatt, finalUuid, 500);
